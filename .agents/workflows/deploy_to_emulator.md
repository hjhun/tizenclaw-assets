...
# Deploy TizenClaw-Assets to Emulator
...
3. **Push and Install the RPM File**
   Transfer the latest built TizenClaw-Assets package to the device's `/tmp/` path and force install it.
   ```bash
   ...
   sdb push ~/GBS-ROOT/local/repos/tizen/${ARCH}/RPMS/tizenclaw-assets-1.0.0-1.${ARCH}.rpm /tmp/
   sdb shell rpm -Uvh --force /tmp/tizenclaw-assets-1.0.0-1.${ARCH}.rpm
   ```
...
4. **Restart Daemon and Check Status**
   ...
   sdb shell dlogutil tizenclaw-ocr
   ```
